...
export default function LegalDocs({ type }) {
  ...
  return <main className="legal-shell">
    ...
    <article>
      <p className="eyebrow">MECHLAB ALPHA · LAST UPDATED AUGUST 17, 2026</p>
      <h1>{privacy ? 'Privacy Policy' : 'Alpha Terms'}</h1>
...
function Privacy() { return <>
  <h2>What MechLab collects</h2><p>Your account uses Supabase Auth. We store the account email needed to authenticate you, your player profile, and alpha access state. When cloud sync is enabled, we also store session summaries and mechanic progress for your account.</p>
  ...
  <h2>Service providers</h2><p>MechLab uses service providers including Supabase for authentication and data services, Cloudflare for website and delivery infrastructure, and Resend for transactional email delivery. Those providers process information needed to operate their respective services.</p>
  <h2>Retention and deletion</h2><p>We retain account and synced product data while your alpha account is active or while reasonably needed to operate and secure the service. You may contact support to request account or synced-data deletion.</p>
  <h2>Security</h2><p>MechLab uses authenticated access controls and row-level security for account data. No internet service can guarantee absolute security, so alpha users should avoid submitting information unrelated to MechLab training.</p>
  <h2>Changes</h2><p>This alpha policy may change as the product changes. Material changes will be reflected by updating the date above.</p>
</> }

function Terms() { return <>
  <h2>Alpha software</h2><p>MechLab is pre-release software provided for testing. Features, scoring thresholds, availability, and data formats may change, and the alpha may contain defects.</p>
  ...
  <h2>Supported use</h2><p>Use the alpha only in supported workflows. Some telemetry features may require optional tooling such as BakkesMod and may be limited to freeplay or offline environments. Do not use MechLab to bypass Rocket League, Psyonix, Epic Games, tournament, or anti-cheat rules.</p>
  ...
  <h2>Availability</h2><p>Alpha access can be changed, suspended, or discontinued as needed for testing, security, abuse prevention, or product changes.</p>
  <h2>Feedback</h2><p>If you submit alpha feedback, you permit MechLab to use that feedback to improve the product without an obligation to compensate you.</p>
</> }
